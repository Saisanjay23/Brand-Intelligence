...
keywords set, running a discovery sweep then an analysis catch-up for each,
...
usable session, run discovery, run an analysis catch-up if there's a
backlog, record the result, repeat. Modelled directly on
...
async def _process_client(slot: int, client_id: str) -> Optional[str]:
    ...
    from backend.services.job_service import ANALYSIS, DISCOVERY, job_manager
    ...
    analysis_ok = True
    _slot_state[slot]["phase"] = "analysis"
    try:
        needs_analysis = False
        for platform_id in ready:
            urls = await profiles_db.urls_for(client_id, platform_id, "approved", exclude_analysed=True)
            if urls:
                needs_analysis = True
                break
        if needs_analysis:
            job = job_manager.create(ANALYSIS, client_id, {}, platform=None)
            if job.task:
                await job.task
            analysis_ok = job.status == "done"
    except asyncio.CancelledError:
        raise
    except Exception as e:
        log.error(f"round-robin analysis for {client_id}: {type(e).__name__}: {e}")
        analysis_ok = False
...
async def client_statuses() -> list[dict]:
    ...
    for c in clients:
        ...
        if rotation_snapshot and c["client_id"] in rotation_snapshot:
