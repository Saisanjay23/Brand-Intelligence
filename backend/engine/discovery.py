...
from backend.engine.jobs import Job
from backend.platforms.scan_options import DiscoveryOptions
from backend.shared.config import settings
from backend.shared.logging import get_logger

log = get_logger("engine.discovery")


def _hit_to_fields(hit, platform: str) -> dict:
    """A discovery Hit -> the plain field dict `db.profiles.save` expects."""
    username = ""
    url = hit.url or ""
...
async def run_discovery(job: Job) -> None:
    from backend.engine.jobs import JobManager
    from backend.platforms import registry
