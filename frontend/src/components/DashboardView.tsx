    ...
    const load = async () => {
      try {
        const [total, analysed, approved, rejected, pending] = await Promise.all([
          api.profiles({ client_id: clientId, limit: 1 }),
          api.profiles({ client_id: clientId, phase: "analysis", limit: 1 }),
          api.profiles({ client_id: clientId, status: "approved", limit: 1 }),
          api.profiles({ client_id: clientId, status: "rejected", limit: 1 }),
          api.profiles({ client_id: clientId, status: "pending", limit: 1 }),
        ]);
        if (cancelled) return;
        setStats({
          total: total.total,
        ...
        const perPlatform = await Promise.all(
          platforms.map((p) =>
            api.profiles({ client_id: clientId, platform: p.platform, limit: 1 }).then((r) => [p.platform, r.total] as const),
          ),
        );
