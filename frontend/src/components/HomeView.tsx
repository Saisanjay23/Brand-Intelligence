 ...
 * backend/api/routes_discovery.py), so there is exactly one job per run. */
...
export function HomeView({
  ...
  const handleRunDiscovery = async () => {
    ...
    try {
      const { job_id } = await api.discover({
        client_id: target.id,
  ...
  const handleRunAnalysis = async () => {
    ...
    if (!target) return;
    try {
      const { job_id } = await api.analyse({ client_id: target.id });
      const job = await api.job(job_id);
      onJobs([job]);
    } catch (e) {
      onError((e as Error).message);
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
            ...
            <button
              ...
              title="Saves this client's keywords + cron so it sweeps automatically -- see backend/engine/scheduler.py"
              style={{
                padding: "10px 16px",
                borderRadius: "10px",
                fontSize: "12px",
